...
cysteine-reactivity changes, plus a bulk download of all processed data.
...
as files. A read-only Flask API (polars-backed) exposes the same data for `curl`
but serves no working UI.
...
sheets that label comparisons in opposite directions. The proteome's is S2-1's
`D8A vs. D8C` block, whose labels are reversed relative to its values (it holds
log₂(D8C/D8A), the same convention that makes `D2 vs. D8C` the D8C panel). The
transcriptome's is S2-2 `D8C vs D8A RNA_*`, numerator-first and read as written;
S1-1 has only the four vs-D2 contrasts.
...
`log2_FC` from one column (`proteome.published_log2fc` = `volcano.log2fc`), so
the two cannot drift apart. AFAP1L2 D8C, the one cell with no published value,
draws a gap, not a zero.
...
no published artifact, and averages ratios outside the log, which biases it
upward with effect size. The reactivity dot plot's expression triangles are the
manuscript's own aggregate, the median over technical channels.

The replicate dots over the bar are supporting evidence, not what sets it: one
per technical channel, two per donor, each over its own donor's D2 mean, so they
show measurement spread rather than donor means.

A TMT channel reading exactly 0 means *below detection*, not *absent*. It is
censored at the replicate's limit of detection, so the value is a bound: upper on
the loss when the condition channel was empty, lower on the increase when the D2
reference was. Channels empty on both sides are dropped, and `n_reps` records the
surviving donors.
...
F13A1, HBB and TNFRSF4 are the whole list, annotated from the `censored` flag on
`proteome_replicates`.
...
published fold change; those proteins keep their per-gene view with the caveat
annotated.
...
and interval are one published quantity. `lfcSE` does not fall to zero with
sequencing depth, being dominated at high counts by biological overdispersion.

The replicate points over the bar are separate supporting evidence, from raw
counts normalized with median-of-ratios size factors. **Do not read their spread
as the error bar**: the SEM of three treatment samples about their own mean
ignores the D2 reference's own uncertainty and runs ~1.9× under `lfcSE`.

Whole-proteome bars *do* carry a replicate SEM, S2-1 publishing no standard error
to use instead. Those points are already per-donor ratios, so the reference sits
inside every one, but read it as the spread of the plotted channels rather than
the bar's uncertainty: pooling a donor's two technical channels as independent
makes it narrower than a donor-level SEM.
...
pointer files where the tables should be and `Store()` refuses to start.
...
Building and serving that tree is the only way to preview the UI: the frontend
fetches pre-rendered JSON under `site/api/` and never talks to a live server.
`python portal/app.py` and `docker compose up --build` run the JSON API alone, so
the page they hand out loads empty; use them with `curl`. GitHub Pages, not
Docker, is the real deployment.
...
routes the volcanoes are dataset-wide: every gene for one comparison, each point
clickable through to its per-gene view. `/?gene=MAP2K4` deep-links a gene.
